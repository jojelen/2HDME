 ...
 *
 * OBS! It takes a looong time to compile this file, so try to avoid making
 * changes to it.
 *============================================================================*/
#include "RGE.h"
...
int rgeFuncThdm_1loop(double t, const double y[], double dydt[], void *params) {
    ...
    Matrix3cd eta1U(3, 3), eta2U(3, 3), eta1D(3, 3), eta2D(3, 3), eta1L(3, 3),
        eta2L(3, 3);
...
#include "RGEs/lambda7_2loop_cForm.h"
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++) {
            dydt[8 + i * 3 + j] =
                pif * real(Deta1U_1(i, j)) + pif * pif * real(Deta1U_2(i, j));
            dydt[17 + i * 3 + j] =
                pif * imag(Deta1U_1(i, j)) + pif * pif * imag(Deta1U_2(i, j));
            dydt[26 + i * 3 + j] =
                pif * real(Deta1D_1(i, j)) + pif * pif * real(Deta1D_2(i, j));
            dydt[35 + i * 3 + j] =
                pif * imag(Deta1D_1(i, j)) + pif * pif * imag(Deta1D_2(i, j));
            dydt[44 + i * 3 + j] =
                pif * real(Deta1L_1(i, j)) + pif * pif * real(Deta1L_2(i, j));
            dydt[53 + i * 3 + j] =
                pif * imag(Deta1L_1(i, j)) + pif * pif * imag(Deta1L_2(i, j));
            dydt[62 + i * 3 + j] =
                pif * real(Deta2U_1(i, j)) + pif * pif * real(Deta2U_2(i, j));
            dydt[71 + i * 3 + j] =
                pif * imag(Deta2U_1(i, j)) + pif * pif * imag(Deta2U_2(i, j));
            dydt[80 + i * 3 + j] =
                pif * real(Deta2D_1(i, j)) + pif * pif * real(Deta2D_2(i, j));
            dydt[89 + i * 3 + j] =
                pif * imag(Deta2D_1(i, j)) + pif * pif * imag(Deta2D_2(i, j));
            dydt[98 + i * 3 + j] =
                pif * real(Deta2L_1(i, j)) + pif * pif * real(Deta2L_2(i, j));
            dydt[107 + i * 3 + j] =
                pif * imag(Deta2L_1(i, j)) + pif * pif * imag(Deta2L_2(i, j));
        }
...
#include "RGEs/SM/yL_2loop_cForm.h"
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j) {
            dydt[5 + i * 3 + j] =
                pif * real(DyU_1(i, j)) + pif2 * real(DyU_2(i, j));
            dydt[14 + i * 3 + j] =
                pif * imag(DyU_1(i, j)) + pif2 * imag(DyU_2(i, j));
            dydt[23 + i * 3 + j] =
                pif * real(DyD_1(i, j)) + pif2 * real(DyD_2(i, j));
            dydt[32 + i * 3 + j] =
                pif * imag(DyD_1(i, j)) + pif2 * imag(DyD_2(i, j));
            dydt[41 + i * 3 + j] =
                pif * real(DyL_1(i, j)) + pif2 * real(DyL_2(i, j));
            dydt[50 + i * 3 + j] =
                pif * imag(DyL_1(i, j)) + pif2 * imag(DyL_2(i, j));
        }
