...
SM::SM()
    ...
    static const std::vector<double> ml = {0.000485289396, 0.1024673155,
                                           1.74215};
    // Pole masses // DEBUG
    // static const std::vector<double> mup = {Global::mu, Global::mc,
    // Global::mt};
    // static const std::vector<double> mdn = {Global::md, Global::ms,
    // Global::mb};
    // static const std::vector<double> ml = {Global::me, Global::mmuon,
    // Global::mtau};
...
void SM::print_all() const {
    ...
    tab1.add_row(
        vector<string>{"g2:", stringAuto(_g2), "cosW:", stringAuto(cW)});
    tab1.add_row(
...
void SM::rge_finished() {
    calc_fermionMasses_and_ckm();
    if (_rgeConfig.dataOutput) _files.close_streams();
}
...
void SM::set_model_from_y(const double y[]) {
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j) {
            _yU(i, j) =
                std::complex<double>(y[5 + i * 3 + j], y[14 + i * 3 + j]);
            _yD(i, j) =
                std::complex<double>(y[23 + i * 3 + j], y[32 + i * 3 + j]);
            _yL(i, j) =
                std::complex<double>(y[41 + i * 3 + j], y[50 + i * 3 + j]);
        }
}
...
void SM::write_slha_file(const std::string &file) const {
    ...
    if (_rgeResults.evolved) {
        ...
        fprintf(
            ...
            _rgeResults.ef);
        fprintf(output,
                " 2   % 16.8e   # Perturbativity breakdown scale (-1 = no "
                "violation)\n",
                _rgeResults.ef_pert);
        fprintf(
            output,
...
void SM::write_to_data_files(const double &t) {
    string fileName =
        _rgeConfig.twoloop ? "smEvolution_2loop.csv" : "smEvolution_1loop.csv";

    if (!_rgeResults.evolved) {
        _files.set_directory("output/" + _rgeConfig.evolutionName);
        _files.create_file(fileName);
        std::vector<std::string> csvHeader;
        csvHeader.push_back("mu");
        csvHeader.push_back("g1");
        csvHeader.push_back("g2");
        csvHeader.push_back("g3");
        csvHeader.push_back("lambda");
        csvHeader.push_back("v");
        csvHeader.push_back("v2");

        _files.add_line(fileName, csvHeader);
        _rgeResults.evolved = true;
    }

    std::vector<double> csvLine;
    csvLine.push_back(exp(t));
    csvLine.push_back(_g1);
    csvLine.push_back(_g2);
    csvLine.push_back(_g3);
    csvLine.push_back(_lambda);
    csvLine.push_back(std::sqrt(_v2));
    csvLine.push_back(_v2);

    _files.add_line(fileName, csvLine);
}
}  // namespace THDME
