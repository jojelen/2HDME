 ...
 *
 * This is the main class of 2HDME.
 * Initialize a THDM object by
 * 1) Set the SM parameters.
 * 2) Set the scalar potential.
 * 3) Set the Yukawa sector.
 *
 * Derived class:
...
class THDM : public RgeModel {
   public:
     ...
     */
    bool set_param_gen(const Base_generic &in,
     ...
     *
     * Enforces an exact Z_2 symmetry and sets the symmetry breaking parameters
     * to
     * zero.
     *
     ...
     *
     * After fixing the kF matrices to be diagonal with the fermion masses, it
     * sets rF = aF*kF.
     */
     ...
     * overwrites it.
     */
    void set_yukawa_manual(const Eigen::Matrix3cd &rU,
                           const Eigen::Matrix3cd &rD,
                           const Eigen::Matrix3cd &rL);
    void set_yukawa_eta(const Eigen::Matrix3cd &eta1U,
    ...
    /**
     * @brief: Checks if the model is a global minimum
     *
     * Performs the algorithm of arXiv:1507.05100 to check whether the current
     * vacuum corresponds to a global minimum.
     *
     */
    bool is_global_minimum() const;
    ...
    /**
     * @brief: Retrieves the aligned Yukawa parameter.
     *
     * Calculates the proportionality coefficient from the 3,3 component of the
     * Yukawa matrices, i.e.
     *   rU(2,2)/kU(2,2), and same for D and L.
     * Note that the Yukawa sector isn't necessarily aligned.
     */
     ...
     * arXiv:1011.6188.
     *
     * get_qij: Returns all 8 components as a vector.
     * get_qki: This method returns the invariants q_{ki} hep-ph/0602242
    ...
    std::vector<std::complex<double>> get_qij()
        const;  // (qi1, qi2) 8 components
     ...
     *
     * These return the Feynman rule coupling of neutral Higgses to fermions,
     * i.e. there is an extra factor of i in it. So, given
     *
     *     L = - g_huu \bar{u}u h - g_huu_axial \bar{h} i\gamma_5 u h,
     *
     * these return (-i*g_huu) or (-i*g_huu_axial) for example. These are 3x3
     * complex matrices.
     *
     * The 3x3 matrix is the coupling of (h_i ff) and is diagonal in cases of no
     * FCNCs.
     *
     * axial: Coupling matrices of (h_i f igamma_5 f).
     */
     ...
     *
     *   L = a_i h_i ( 2 (mW^2) WW + mZ^2 ZZ)/v.
     *
     *  get_VV_couplings: Returns a_i.
     *  get_hvv: Returns the full Feynman diagram coupling.
     *  get_coupling_vvh: Calls get_hvv to set c.
     */
    ...
    std::complex<double> get_rFij(FermionSector fermion, int i, int j) const;
     ...
     * They are returned as tuples, with the first component being the name of
     * the
     * element as a string.
     */
    std::tuple<std::string, int, std::complex<double>> get_largest_diagonal_rF()
        const;
    ...
    std::complex<double> get_lamF_element(const FermionSector flavor,
 ...
 */
     ...
     */
    void write_slha_file(
        const int sphenoLoopLvl,
    ...
    // 2HDMC functions
    //
    // To be able to use 2HDMC classes such as DecayTable, the following
    // functions are implemented. They are similar as the 2HDMC functions with
    // equal names, but are modified to give the correct answer in the general
    // case.
    //---------------------------------------------------------------------------
    ...
    void get_coupling_hhh(int h1, int h2, int h3,
   ...
   private:
     ...
     *
     *  Fixes M112, M222 and the phase xi if beta is non-zero.
    ...
    // Functions that are called in RG evolution and produces data output to
    // files
   ...
   private:
    ...
    // Stores the filename for csv file containing evolution of parameters
    std::string _csvFile;
